...
fn_determine_character = determine_character = lambda: (
    (lambda scores: (random.choice([c for c, s in scores.items() if s == max(scores.values())]), scores))(
        Counter({char: score for q_idx, a_idx in enumerate(st.session_state.answers) for char, score in QUESTIONS[q_idx]["scores"][a_idx].items()})
    )
)
